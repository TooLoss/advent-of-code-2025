...
std::pair<ULONGI, ULONGI> intersection_range(const std::vector<std::pair<ULONGI, ULONGI>> &database, int begin, int end) {
...
std::vector<std::vector<size_t>> get_all_segment(size_t n) {
...
ULONGI card_union_database(std::vector<std::pair<ULONGI, ULONGI>> &database) {
    ULONGI card_total = 0;
    auto all_segment = get_all_segment(database.size() - 1);
    for (const auto I : all_segment) {
        const auto intersection = database[I[0]];
        for (const auto value : I) {
            intersection = intersection(intersection, database[]);
            ULONGI card_current = 0;

            if (inter_current.first != static_cast<ULONGI>(-1) && inter_current.second != static_cast<ULONGI>(-1))
                card_current = inter_current.second - inter_current.first;

            int sign = (pair.second - pair.first) % 2 == 0 ? 1 : -1;
